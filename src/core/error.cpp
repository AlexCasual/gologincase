...
{ // anonymous namespace
	...
	{
	  switch (static_cast<gologin::core::errc>(ev))
	  {
	  case gologin::core::errc::fail:
		return "common core error";
	   
	  case gologin::core::errc::success:
...
{	
	std::error_code make_error_code(gologin::core::errc e)
	{
		 return {static_cast<int>(e), the_core_error_category};
	}
	
	exception::exception(char const* const what, const gologin::core::errc& ec)
	: m_what("gologin::core::exception => "), m_errc(ec)
	{
		m_what += "[";
		m_what += what;
		m_what += "]";
	...
	const gologin::core::errc& exception::code() const
	{
		return m_errc;
	}
}