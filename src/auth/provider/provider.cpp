...
namespace gologin::auth
{
	...
	client_status provider::add_client(std::uint64_t _client_id, const std::string& _client_login, const std::string& _client_pass, client_session_id_t& _session)
	{
		...
		if (auto _c = m_clients.find(_client_id); _c == m_clients.end())
		{
			...
			m_clients.emplace(std::piecewise_construct,
				...
				std::forward_as_tuple(_client_id, _session, client_status::accepted));
